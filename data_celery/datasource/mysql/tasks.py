...
@celery_app.task(name="collection_mysql_task")
def collection_mysql_task(task_uid: str, user_name: str, user_token: str):
    """
    Collection task
    Args:
    ...
    try:
        logger.info(f"Starting the task[{task_uid}]...")
        ...
        if not extra_config:
            ...
            return False
        # Read configuration
        if "mysql" not in extra_config:
            collection_task.task_status = DataSourceTaskStatusEnum.ERROR.value
            insert_datasource_run_task_log_error(task_uid, f"Task with UID {task_uid} has no MySQL configuration.")
            return False
        mysql_config = extra_config["mysql"]
        use_type = ""
        use_sql = ""
        max_line = 10000
        csg_hub_dataset_id = ''
        csg_hub_dataset_default_branch = "main"
        if "type" in mysql_config:
            use_type = mysql_config["type"]
        if "sql" in mysql_config:
            use_sql = mysql_config["sql"]
        ...
        if use_type == "sql":
            ...
            # Obtain data according to mysql_config
            if "source" in mysql_config:
                source = mysql_config["source"]
                # First collect the total of all tables
                total_count = 0
                records_count = 0
                connector = get_datasource_connector(collection_task.datasource)
                if not connector.test_connection():
                ...
                for table_name in source.keys():
                    table_total = connector.get_table_total_count(table_name)
                    total_count += table_total

                collection_task.total_count = total_count
                collection_task.records_count = records_count
                logger.info(f"Task with UID {task_uid} has collected {total_count} records.")
                db_session.commit()
                for table_name in source.keys():
                    config_columns = source[table_name]
                    get_table_dataset(connector, task_uid, db_session, collection_task, table_name, config_columns,
                                      datasource_temp_parquet_dir, max_line=max_line)
...
def get_table_dataset(connector,
                      task_uid: str,
                      db_session: Session,
    ...
    try:
        # Get all current lists and retrieve the real column names
        real_get_columns = []
        ...
        while True:
            ...
            rows = connector.query_table(table_name, real_get_columns, offset=(page - 1) * page_size,
                                         limit=page_size)
            if not rows:
                break  # If there is no more data, exit the loop
            # Add the query results to the buffer list

...
def init_configs(args=None,redirect=True):
    ...
    parser.add_argument('--user_id',
                        type=str,
                        default=None,
                        help='user id of download/upload dataset.')
    parser.add_argument('--user_name',
                        type=str,
                        default=None,
                        help='user name of download/upload dataset.')
    parser.add_argument('--user_token',
                        type=str,
                        default=None,
                        help='user token of download/upload dataset.')
    parser.add_argument(
        '--repo_id',
        type=str,
        help='repo_id of dataset from csghub.',
        required=False)
    parser.add_argument(
        '--branch',
        type=str,
        help='branch of dataset from csghub.',
        required=False)
...
def sort_tool_by_types_and_names(tool_name_classes):
    """
    Split tools items by tool type and sort them to sub-tools by name, then concat
    together.

    :param tool_name_classes: a list of tool modules
    :return: sorted op list , each item is a pair of tool_name and
        tool_class
    """
    preprocess_tools = [(name, c) for (name, c) in tool_name_classes
                        if 'preprocess' in name]
    postprocess_tools = [(name, c) for (name, c) in tool_name_classes
                         if 'postprocess' in name]
    common_tools = [(name, c) for (name, c) in tool_name_classes
                    if 'common' in name]

    tools_sorted_by_types = sorted(common_tools) + sorted(preprocess_tools) + sorted(
        postprocess_tools)
    
    return tools_sorted_by_types

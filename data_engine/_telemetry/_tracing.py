...
from ._tracing_config import ExtraAttributes, Operation, TracingConfig
from ._propagation import TelemetryMetadataContainer, get_telemetry_context


class TraceHelper(Generic[Operation, ExtraAttributes]):
    """
    ...
    def __init__(
        ...
        instrumentation_builder_config: TracingConfig[Operation, ExtraAttributes],
    ) -> None:
        self.tracer = (tracer_provider if tracer_provider else NoOpTracerProvider()).get_tracer(
            f"dataflow {instrumentation_builder_config.name}"
        )
    ...
    def trace_block(
        ...
        parent: Optional[TelemetryMetadataContainer] = None,
        *,
        ...
        """
        span_name = self.instrumentation_builder_config.get_span_name(operation)

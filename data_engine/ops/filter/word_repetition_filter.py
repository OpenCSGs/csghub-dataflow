...
class WordRepetitionFilter(Filter):
    ...
    @property
    def description(cls):
        return """Filter to keep samples with word-level n-gram repetition ratio within a
    specific range."""

    @classmethod
    @property
    def sample(cls):
        return Sample("根据算子使用使用使用使用安装方案确定", "")
    ...
    def init_params(cls):
        return [
            ...
            Param("rep_len", DataType.PositiveFloat, None, 10),
            Param("min_ratio", DataType.ClosedUnitInterval, None, 0.0),
            Param("max_ratio", DataType.ClosedUnitInterval, None, 0.5),
        ]
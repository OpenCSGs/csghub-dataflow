...
from data_engine.ops.base_op import OPERATORS,Sample,Param, DataType,OP
from data_engine.ops import Filter
...
OP_NAME = 'pipeline_magpie_zh'

@OPERATORS.register_module(OP_NAME)
class PipelineMagpieZh(OP):
    def __init__(self,
                 api_key ="sk-119d00ab5e454d46a1a48ff18ded9e64",
                 # model = "qwen-plus,qwen2.5-14b-instruct-1m,deepseek-r1",
                 model="deepseek-r1",
    ...
    def run(self, dataset, **kwargs):
        """
        This operator is a generator. It ignores the input dataset and creates
        a new one from scratch by calling an LLM API.
        """
        all_samples = []
        ...
        for category_name, (system_prompt, weight) in self.content.items():
            generation_instruction = (
                f"请基于以下系统提示词和对应权重生成一个3-5轮的中文对话。"
                ...
                f"助手：[助手回答]\n"
                f"...\n\n"
                f"系统提示词：{system_prompt},对应权重：{weight}"
            )
            ...
            try:
                ...
                print(f"发送消息 {generation_instruction}")
                print(f"返回消息 {result.choices[0].message.content}")
                generated_text = result.choices[0].message.content
                all_samples.extend(self._parse_conversation(generated_text))
                print(f"成功生成 {category_name} 类别的对话")

            except Exception as e:
                print(f"生成 {category_name} 类别对话时出错: {str(e)}")

        # Create a new dataset from the generated samples
        from datasets import Dataset as HFDataset
        from data_engine.core.data import NestedDataset
        if not all_samples:
            # If generation fails, return the original dataset to not break the pipeline
            return dataset

        hf_dataset = HFDataset.from_list(all_samples)
        return NestedDataset(hf_dataset)
    ...
    def process(self, sample):
        # This method is required by the base OP class, but since this operator
        # is a generator that overrides run(), this method is not used.
    ...
    def init_params(cls):
        return [
            Param("api_key", DataType.STRING, {}, "http://ev4spm6lel8g.spaces.opencsg.com/v1"),
            Param("model", DataType.STRING, {}, "qwen-plus"),
            Param("base_url", DataType.STRING, {}, "https://dashscope.aliyuncs.com/compatible-mode/v1"),
            Param("content", DataType.STRING, {}, CATEGORIES_SYSTEM_PROMPTS)
        ]
...
#
#     # This operator is a generator, so it ignores the input dataset.
#     # We create a dummy empty dataset to pass to the run method, which is
#     # what the execution engine would do.
#     initial_hf_dataset = Dataset.from_dict({})
#     initial_dataset = NestedDataset(initial_hf_dataset)
#
#     magpie_zh = PipelineMagpieZh()
#
#     # Call the run method, which contains the generation logic
#     result_dataset = magpie_zh.run(initial_dataset)
#
#     # Print the contents of the generated dataset
#     print("Generated dataset:")
#     print(result_dataset)
#
#     # To see the actual data, you can convert it to a list of dicts

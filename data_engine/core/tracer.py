...
@ray.remote
def ray_ds_2_list(data: Dataset):
...
class Tracer:
    ...
    def generate_count(func):
        def wrapper(*args, **kwargs):
            ...
            tracer = args[0]
            op_name = args[1]
            if isinstance(args[2], ray.data.dataset.Dataset):
                # original_ds = args[2].take_all()
                original_ds = ray.get(ray_ds_2_list.remote(args[2]))
                original_count = len(original_ds)
                args_list[2] = original_ds
            else:
                original_count = len(args[2])

            res_name = f'count-{op_name}.txt'
            try:
                file = open(os.path.join(tracer.work_dir, res_name), "w")
                file.write(str(original_count))
            except IOError as e:
                logger.error(f'{e}: write trace count file failed')
            finally:
                file.close()
            

            func(*tuple(args_list), **kwargs)
        return wrapper
    ...
    @generate_count
    def trace_batch_mapper(self, op_name: str, previous_ds: Union[Dataset, List[Dict[str, Any]]],
                           processed_ds: Union[Dataset, List[Dict[str, Any]]], text_key: str):
        """

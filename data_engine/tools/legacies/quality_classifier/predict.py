...
from data_engine.tools.legacies.quality_classifier.qc_utils import (export_result, init_spark,
                                               load_dataset, predict,
                                               prepare_model, load_datasets)
...
def predict_score(dataset_path,
    ...
    # load dataset
    # List all file with suffixes '.json, .jsonl, .parquet' in the directory
    if not os.path.exists(dataset_path):
        logger.error(f"Dataset path {dataset_path} does not exist.")

    # list files with .json, .jsonl, .parquet
    dataset_files = [os.path.join(dataset_path, f) for f in os.listdir(dataset_path) if f.endswith(('.json', '.jsonl', '.parquet'))]

    logger.info(f"Found files: {dataset_files}")
    # ds = load_dataset(spark, dataset_path, text_key=text_key)
    ds = load_datasets(spark, dataset_files, text_key=text_key)
